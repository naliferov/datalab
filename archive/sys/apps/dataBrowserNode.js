async () => {
    ...
    return class dataBrowserNode {
        ...
        async open(openedPaths = {}) {
            ...
            const DataBrowserNodeClass = await s.f('sys.apps.dataBrowserNode');

            const renderNode = async (k, node) => {
                const dataNode = new NodeClass(node);
                dataNode.setKey(k);

                const dataBrowserNode = new DataBrowserNodeClass;
                await dataBrowserNode.init(dataNode, false, this.dataBrowser);
                e('>', [dataBrowserNode.getV(), this.getNodesV()]);
                this.dataBrowser.addNode(dataBrowserNode);

                if (openedPaths[k] && dataBrowserNode.hasSomethingToOpen()) {
                    await dataBrowserNode.open(openedPaths[k]);
                }
            }
